...
class DWSConvLSTM2d(nn.Module):
    ...
    def forward(self, x: th.Tensor, h_and_c_previous: Optional[Tuple[th.Tensor, th.Tensor]] = None) \
        ...
        """
        if h_and_c_previous is None: # 对应一开始的时候，没有h_previous和c_previous，就自动的生成
            # generate zero states
            hidden = th.zeros_like(x)
            cell = th.zeros_like(x)
            h_and_c_previous = (hidden, cell) # 组织成一个tuple的形式
        h_tm1, c_tm1 = h_and_c_previous

        # 如果conv_only_hidden=True, hidden state会通过conv3x3进行卷积
        # 然后和输入x拼接，拼接以后只会有一个1x1的卷积
        # 如果False，首先concate在一起的结果会进行3x3的depth wise conv
        # 还会额外的再进行一个1x1的卷积
        if self.conv_only_hidden:
            h_tm1 = self.conv3x3_dws(h_tm1) # 对hidden_state首先用一个深度可分离卷积
        xh = th.cat((x, h_tm1), dim=1) # 输入concate上hidden state h_tml
        if not self.conv_only_hidden:
            xh = self.conv3x3_dws(xh) # concate以后一起进行一个深度可分离卷积
        mix = self.conv1x1(xh) # 再对concate的结果应用1x1 conv
